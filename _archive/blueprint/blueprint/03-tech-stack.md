...
### Execution Layer
- `websockets` — Raw WebSocket client for Deriv's **new PAT/Options API** (PAT → OTP endpoint → authenticated WS URL). No SDK needed.
- ~~`python-deriv-api`~~ — **Removed (2026-08-02).** Deriv migrated to PAT tokens; the legacy library's `authorize` flow returns "token invalid" and is unusable.
...
- [ ] Test outbound connection: `wscat -c wss://api.derivws.com/trading/v1/options/ws/public`
