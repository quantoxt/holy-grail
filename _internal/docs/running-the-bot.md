...
Production runs on the **Windows VPS** (`aurora@192.168.0.179`) because the `MetaTrader5`
...
ssh aurora@192.168.0.179 'taskkill /F /IM pythonw.exe'

# START
ssh aurora@192.168.0.179 'schtasks /run /tn HolyGrail'

# CLEAN RESTART (the safe 3-step — use this after deploys)
ssh aurora@192.168.0.179 'taskkill /F /IM pythonw.exe'
ssh aurora@192.168.0.179 'tasklist /fi "imagename eq pythonw.exe"'   # MUST show "No tasks"
ssh aurora@192.168.0.179 'schtasks /run /tn HolyGrail'
```
...
scp soldier/loop.py aurora@192.168.0.179:"C:/Users/Aurora/Documents/PROJECTS/holy-grail/soldier/loop.py"
#   (repeat per file; preserve the subdirectory path)

# 3. clean restart (the 3-step above)

# 4. verify the new code loaded
ssh aurora@192.168.0.179 'powershell -NoProfile -Command "Select-String -Path C:\Users\Aurora\Documents\PROJECTS\holy-grail\soldier\loop.py -Pattern \"SOME_NEW_FUNCTION\" -Quiet"'
```
...
```bash
ssh aurora@192.168.0.179 'tasklist /fi "imagename eq pythonw.exe" /fo csv /nh && powershell -NoProfile -Command "Get-Content C:\Users\Aurora\Documents\PROJECTS\holy-grail\data\bot.log -Tail 5 -Encoding UTF8"'
```
